...
onready var camguide_y = get_parent().get_node("character1_controller/camguide_y")
onready var camguide_x = get_parent().get_node("character1_controller/camguide_y/camguide_x")
...
func _process(delta):
	...
	if joystick_active:
		if ((camera_index !=1  and sign(camera_input.x) == sign(joystick_vector.x)) 
		or camera_index == -1) and current_state == "run":
			auto_rotation = lerp(auto_rotation, joystick_vector_normalized.x * 2, delta *10)
...
func _on_character1_controller_state(state):
	current_state = state
